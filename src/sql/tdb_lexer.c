...
static const kw k_keywords[] = {
  ...
  /* New reserved keywords for Phase 11. Strategy notes (RANGE/LIST/HASH),
  ** placement targets (TABLESPACE, COMPRESSION, LOCATION), authorization verbs
  ** (GRANT/REVOKE), and shape-defining clauses (PARTITION, TRUNCATE, etc.)
  ** are full keywords. Identifier-friendly words (ROLE, USER, TYPE, OWNED,
  ** PRIVILEGES, RANGE, LIST, HASH) are kept lexed as TK_ID and matched by
  ** text in the parser so common column/table names are not broken. */
  {"TABLESPACE",TK_TABLESPACE},{"PARTITION",TK_PARTITION},
  {"TRUNCATE",TK_TRUNCATE},{"ANALYZE",TK_ANALYZE},{"REINDEX",TK_REINDEX},
  {"COMMENT",TK_COMMENT},{"COMPRESSION",TK_COMPRESSION},
  {"GRANT",TK_GRANT},{"REVOKE",TK_REVOKE},
  {"ATTACH",TK_ATTACH},{"DETACH",TK_DETACH},{"DATABASE",TK_DATABASE},
  {"LOCK",TK_LOCK},{"LOCATION",TK_LOCATION},
};
...
static const char *const k_all_keywords[] = {
  ...
  "ASC","ATTACH","AUTOINCREMENT","BEFORE","BEGIN","BETWEEN","BIT","BY","CALL",
  "CASCADE","CASE","CAST","CHECK","COLLATE","COLUMN","COMMENT","COMMIT",
  "COMPRESSION","CONFLICT","CONSTRAINT","CREATE","CROSS","CURRENT",
  "CURRENT_DATE","CURRENT_TIME","CURRENT_TIMESTAMP","DATABASE","DEFAULT",
  "DEFERRABLE","DEFERRED","DELETE","DESC","DETACH","DISTINCT","DO","DROP",
  "EACH","ELSE","ELSIF","END","ESCAPE","EXCEPT","EXCLUDE","EXCLUSIVE","EXISTS",
  "EXPLAIN","FAIL","FILTER","FIRST","FOLLOWING","FOR","FOREIGN","FROM","FULL",
  "FUNCTION","GENERATED","GLOB","GRANT","GROUP","GROUPS","HASH","HAVING","IF",
  "IGNORE","IMMEDIATE","IN","INDEX","INDEXED","INITIALLY","INNER","INSERT",
  "INSTEAD","INTERSECT","INTERVAL","INTO","IS","ISNULL","JOIN","KEY","LANGUAGE",
  "LAST","LEFT","LIKE","LIMIT","LIST","LOCATION","LOCK","LOOP","MATCH",
  "MATERIALIZED","NATURAL","NO","NOT","NOTHING","NOTNULL","NULL","NULLS","OF",
  "OFFSET","ON","OR","ORDER","OTHERS","OUTER","OVER","OWNED","PARTITION",
  "PERIOD","PLAN","PRAGMA","PRECEDING","PREPARE","PRIMARY","PRIVILEGES",
  "PROCEDURE","QUERY","RAISE","RANGE","RECURSIVE","REFERENCES","REGEXP",
  "REINDEX","RELEASE","RENAME","REPLACE","RESTRICT","RETURNING","RETURNS",
  "REVOKE","RIGHT","ROLE","ROLLBACK","ROW","ROWS","SAVEPOINT","SELECT",
  "SEQUENCE","SET","STORED","SYSTEM","TABLE","TABLESPACE","TEMP","TEMPORARY",
  "THEN","TIES","TO","TRANSACTION","TRIGGER","TRUNCATE","TYPE","UNBOUNDED",
  "UNION","UNIQUE","UPDATE","USER","USING","VACUUM","VALUES","VARBIT",
  "VERSIONING","VIEW","VIRTUAL","WHEN","WHERE","WHILE","WINDOW","WITH",
  "WITHOUT","XML",
};
